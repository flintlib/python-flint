...
cdef class fmpq_mpoly_ctx(flint_mpoly_context):
    ...
    Do not construct one of these directly, use `get_fmpq_mpoly_context`.
    """
    ...
    def fmpq_mpoly_from_dict(self, d):
         ...
         to fmpz) representing exponents, and corresponding values of fmpq.
         ...
         for k,v in d.items():
             ...
             if not fmpq_is_zero(coefficient):
                 # fmpq_mpoly_push_term_fmpq_fmpz(res.val, coefficient, exponents, self.val)
                 _fmpz_mpoly_push_exp_ffmpz(res.val.zpoly, exponents, self.val.zctx)
                 fmpq_mpoly_set_term_coeff_fmpq(res.val, count, coefficient, self.val)
                 count += 1
         for i in range(nvars):
...
cdef class fmpq_mpoly(flint_mpoly):
    """
    The *fmpq_poly* type represents sparse multivariate polynomials over
    the integers.
    """
    ...
    def __init__(self, val=0, ctx=None):
        if typecheck(val, fmpq_mpoly):
            ...
            #XXX this copy is silly, have a ctx function that assigns an fmpq_mpoly_t
            ...
            if v is NotImplemented:
                raise TypeError("cannot create fmpq_mpoly from type %s" % type(val))
            if ctx is None:
                raise ValueError("Need context to convert  fmpq to fmpq_mpoly")
            init_fmpq_mpoly(self, ctx)
